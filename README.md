...
Plataforma de treino gamificado para vestibulares, com 15 disciplinas, simulados, repetição espaçada, cálculo rápido, progressão contínua, recursos sociais e PWA.
...
- A busca social usa uma projeção mínima em `public_profiles/{uid}` e respeita a privacidade escolhida.
- XP competitivo só é concedido depois que o servidor valida uma questão emitida pelo próprio servidor, de uso único.
- Sincronização exige token Firebase e nunca aceita identidade ou XP competitivo enviados pelo navegador.
- Desafios e notificações são validados pela API autenticada.
- A exclusão de conta remove autenticação, perfil privado/público e relações associadas.
...
A API Express é executada como uma Vercel Function. Ranking, questões verificadas, idempotência e gabaritos privados usam o Firestore; nenhum dado depende do disco temporário da Function. Para limpeza automática, é recomendável habilitar TTL no Firestore para o campo `expiresAt` das coleções `competitive_questions` e `processed_events`.
