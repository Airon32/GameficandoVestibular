...
export const VirtualKeypad: React.FC<VirtualKeypadProps> = ({
  ...
  return (
    <div className="w-full max-w-lg sm:max-w-xl md:max-w-2xl mx-auto select-none px-1 sm:px-2 touch-manipulation">
      <div className="virtual-keypad-grid grid grid-cols-3 gap-2 sm:gap-4 md:gap-5">
        ...
        {numpadButtons.map((digit) => (
          <button
            ...
            className="virtual-keypad-button h-[clamp(4.25rem,8dvh,5.5rem)] sm:h-24 md:h-26 rounded-2xl sm:rounded-3xl bg-[#181818] hover:bg-[#252525] active:bg-[#303030] active:scale-[0.96] text-4xl sm:text-5xl md:text-6xl font-mono font-black text-white border-2 border-[#282828] hover:border-[#3a3a3a] transition-all flex items-center justify-center cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shadow-xl"
          >
            {digit}
          </button>
        ))}
        ...
        <button
          ...
          className="virtual-keypad-button h-[clamp(4.25rem,8dvh,5.5rem)] sm:h-24 md:h-26 rounded-2xl sm:rounded-3xl bg-[#261515] hover:bg-[#351a1a] active:bg-[#451f1f] active:scale-[0.96] text-red-400 border-2 border-red-900/50 hover:border-red-700/70 transition-all flex items-center justify-center cursor-pointer disabled:opacity-40 shadow-xl"
        ...
        <button
          ...
          className="virtual-keypad-button h-[clamp(4.25rem,8dvh,5.5rem)] sm:h-24 md:h-26 rounded-2xl sm:rounded-3xl bg-orange-600 hover:bg-orange-500 active:bg-orange-700 active:scale-[0.96] text-white shadow-2xl shadow-orange-950/70 border-2 border-orange-400/60 transition-all flex items-center justify-center cursor-pointer disabled:opacity-40"
      ...
      <div className="virtual-keypad-helper mt-1.5 sm:mt-4 flex min-h-6 items-center justify-between text-[11px] sm:text-xs text-[#777] px-2 font-semibold uppercase tracking-wider">
        <span className="hidden sm:inline">Teclado: 0-9 • Backspace • Enter</span>
        <button
          type="button"
          onClick={() => handleKeyInteraction(() => onClearPress())}
          aria-label="Limpar todos os números"
